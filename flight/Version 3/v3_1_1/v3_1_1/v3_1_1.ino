...
float getAltitude(uint32_t reading_time){
      baro.read();
      if (!baro.healthy) {
          hal.console->println("not healthy");
          return 0.0;
...
void loop() 
{
  ...
  uint32_t read_time = hal.scheduler->micros() - interval;
  if((hal.scheduler->micros() - interval) > 100000UL) {
    alt = getAltitude(read_time);
...
//    hal.scheduler->delay(1000);
    ...
    long alt_output = (long) constrain(pids[ALT_RATE].get_pid(alt_stab_output - climbRate, 1), -100, 100);
    ...
    //hal.console->print(rcthr + roll_output + pitch_output - yaw_output);
    ...
    rcthr = map(alt_output, -100, 100, 1100, 1250);
