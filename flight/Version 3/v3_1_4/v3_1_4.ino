...
void loop() 
{
  ...
  rcthr = map(channels[2], RC_THR_MIN, RC_THR_MAX, RC_THR_MIN, 1400);
  rcalt = 0.75; //Hard code in RCALT
  ...
  if(rcalt >= 0) {  // Altitude raised, turn on stablisation.
    ...
    float alt_output = constrain(pids[ALT_STAB].get_pid((float)rcalt - alt, 1), -50, 50);
    ...
    if (channels[5] < 1200){
      ...
      //Map the Throttle
      rcthr = map(alt_output, -1, 1, 1000, 1600);
      rcthr = constrain(rcthr, 1200, 1340); 
