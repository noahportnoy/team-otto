...
void loop() 
{
  ...
  // Do the magic
  if(rcalt > -1) {  // Throttle raised, turn on stablisation.
    // Stablise PIDS
...
//    hal.console->println();
    ...
    long alt_stab_output = constrain(pids[ALT_STAB].get_pid((float)rcalt - alt, 1), -250, 250);
    long alt_output = (long) constrain(pids[ALT_RATE].get_pid(alt_stab_output - climbRate, 1), -500, 500);
...
//    hal.scheduler->delay(1000);
    ...
    rcthr = constrain(rcthr + alt_output, 1000, 1500);
