...
void loop() 
{
  ...
  while( (AVG_OFF_BUTTON_VALUE < 1.0) || (safety < 1500)){
    ...
    if (gps.new_data) {
        ...
        hal.console->print((float)gps.latitude / 10000000.0, DEC);
        hal.console->print(" Lon:");
        hal.console->print((float)gps.longitude / 10000000.0, DEC);
  ...
  if(rcalt >= 0) {  // Altitude raised, turn on stablisation.
    ...
    //Map the Throttle
    rcthr = map(alt_output, -1, 1, 1000, 1700);
    rcthr = constrain(rcthr, 1200, 1700);
