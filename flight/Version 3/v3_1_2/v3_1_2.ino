...
void setup() 
{
  ...
  pids[ALT_RATE].kP(2.0);
  pids[ALT_RATE].kI(0.1);
...
void loop() 
{
  ...
  //while (ins.num_samples_available() == false);
  ...
  if((hal.scheduler->micros() - interval) > 100000UL) {
    ...
    climb_rate = movingAvg(last_climb_rate, climb_rate, .8);
...
//    hal.console->println();
    ...
    //Map the Throttle
    rcthr = map(alt_output, -5, 5, 800, 1600);
    rcthr = constrain(rcthr, 1100, 1600);
